...
export function EmptyState({ icon: Icon, title, description, action }: EmptyStateProps) {
  const buttonClasses = "px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background";
  ...
  return (
    <div data-ev-id="ev_2c1845ee7b" className="flex flex-col items-center justify-center py-16 px-4 text-center">
        ...
        action.onClick ?
        <button data-ev-id="ev_6cf80b34b9" onClick={action.onClick} className={buttonClasses}>
          {action.label}
        </button> :
        null
      )}
